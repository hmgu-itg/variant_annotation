...
def main():
    ...
    for L in utils.chunks([line.rstrip() for line in sys.stdin.readlines()],config.VEP_POST_MAX):
        ...
        i+=1
        if use_name:
            r=variant.addConsequencesToRSList(L,build=build,most_severe_only=use_max,gene_key="gene_symbol")
        else:
            r=variant.addConsequencesToRSList(L,build=build,most_severe_only=use_max,gene_key="gene_id")
        if r is None:
            continue
        for ID in r:
            for g in r[ID]:
