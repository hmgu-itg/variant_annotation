...
# cat t|sed 's/ class="[^"]*"//g' | sed 's/<\/div><\/div>/\n/g'|sed 's/<div><div><span>/\n<div><div><span>/g' |perl -lne 'print $1 if /^\<div\>\<div\>\<span\>(.*)\<\/div\>\<\/label\>$/;' |perl -lne '@M=($_=~/\<div\>\<span\>(.*?)\<\/span\>\<\/div\>/g);foreach $x (@M){print $x;}'| less

echo $(date '+%d/%m/%Y %H:%M:%S') "Concatenating" $(cat $tmpfile2 | wc -l) "file(s)"
concat.py $(cat $tmpfile2| tr '\n' ' ') | gzip - > $out/GXA.tsv.gz

# echo $(date '+%d/%m/%Y %H:%M:%S') "Splitting"
# splitByGene.py $out/GXA.tsv
